<script lang="ts">
	...
	async function deleteImage(id: string) {
		showConfirm('删除镜像', '确定要删除此镜像吗？', async () => {
			try {
	...
	async function pruneImages() {
		showConfirm('清理未使用镜像', '确定要清理所有未使用的镜像吗？', async () => {
			pruning = true;
			try {
				...
				<Card>
					<div class="flex items-center justify-between">
						...
						<div class="flex items-center gap-6">
							...
							<!-- Containers -->
							<div class="text-right text-sm">
								<div class="text-text-secondary">{image.Containers} 个容器</div>
							</div>

							<!-- Actions -->
							<div class="flex gap-2">
								<Button variant="danger" size="sm" onclick={() => deleteImage(image.Id)} disabled={image.Containers > 0} title={image.Containers > 0 ? '有容器在使用，无法删除' : '删除镜像'}>
									<Trash2 size={14} />
								</Button>
							</div>
						</div>
